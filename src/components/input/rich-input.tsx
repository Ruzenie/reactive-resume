...
export function RichInput({ value, onChange, variant = "full", style, className, editorClassName, ...options }: Props) {
    ...
    editorProps: {
      attributes: {
        ...
        class: cn(
          ...
          isInline
            ? "min-h-9 overflow-hidden bg-transparent px-2.5 py-2 text-sm font-medium leading-tight outline-none"
            : "group/editor overflow-y-auto rounded-md rounded-t-none border p-3 pb-4 outline-none focus-visible:border-ring data-[fullscreen=false]:max-h-[400px] data-[fullscreen=false]:min-h-[100px] data-[fullscreen=true]:max-h-none data-[fullscreen=true]:min-h-full",
          styles.tiptap_content,
          styles.editor_content,
          isInline && styles.inline_editor_content,
          editorClassName,
        ),
      },
  ...
  const editorElement = isInline ? (
    <div className="group/rich-input relative rounded-md border border-input bg-transparent shadow-xs transition-[color,box-shadow] focus-within:border-ring focus-within:ring-3 focus-within:ring-ring/50">
      <EditorContent editor={editor} />
      <EditorToolbar editor={editor} isFullscreen={false} variant={variant} />
    </div>
  ) : (
    <div className="relative">
      <EditorToolbar editor={editor} isFullscreen={isFullscreen} variant={variant} />
...
function EditorToolbar({
  ...
  const toggleSize = isFullscreen ? "lg" : "sm";
  const buttonSize = isInline ? "icon-xs" : isFullscreen ? "lg" : "sm";
  const toolbarClassName = cn(
    isInline
      ? "hidden items-center justify-end gap-0.5 border-t border-border/70 bg-muted/20 px-1 py-1 group-focus-within/rich-input:flex"
      : "flex flex-wrap items-center gap-y-0.5 rounded-md rounded-b-none border border-b-0",
  );
  const dividerClassName = isInline ? "mx-0.5 h-4 w-px bg-border" : "mx-1 h-5 w-px bg-border";
  const toggleClassName = cn(
    isInline
      ? "size-6 min-w-6 rounded-sm px-0 text-muted-foreground hover:text-foreground aria-pressed:bg-muted aria-pressed:text-foreground"
      : "rounded-none",
  );
  const buttonClassName = cn(
    isInline ? "size-6 rounded-sm px-0 text-muted-foreground hover:text-foreground" : "rounded-none",
  );
  const textColorTriggerClassName = cn(
    isInline ? "size-6 rounded-sm px-0 text-muted-foreground hover:text-foreground" : "rounded-none px-2",
      ...
      <PopoverTrigger
        render={
          <Button
            ...
            tabIndex={-1}
            variant="ghost"
            className={textColorTriggerClassName}
            title={t`Text Color`}
            disabled={!state.canTextColor}
          >
            <span className="flex flex-col items-center leading-none">
              <span className={cn("font-semibold", isInline ? "text-[11px]" : "text-xs")}>A</span>
              <span
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-xs text-muted-foreground">
                <Trans comment="Preset or custom shade refer to the color picker">Choose a preset or custom shade.</Trans>
              </span>
            </div>
          </div>

          <Button size="xs" variant="ghost" className="shrink-0" onClick={state.unsetTextColor} disabled={!state.textColor}>
      ...
      {!isInline && (
        <>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              render={
                <Button size={buttonSize} tabIndex={-1} variant="ghost" className={buttonClassName}>
                  {match(state)
                    ...
                    .otherwise(() => <ParagraphIcon className="size-3.5" />)}
                </Button>
              }
            />

            <DropdownMenuContent>
              <DropdownMenuCheckboxItem disabled={!state.canParagraph} checked={state.isParagraph} onCheckedChange={state.setParagraph}>
                <Trans>Paragraph</Trans>
              </DropdownMenuCheckboxItem>
              <DropdownMenuSeparator />
              <DropdownMenuCheckboxItem disabled={!state.canHeading1} checked={state.isHeading1} onCheckedChange={state.toggleHeading1}>
                <Trans>Heading 1</Trans>
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem disabled={!state.canHeading2} checked={state.isHeading2} onCheckedChange={state.toggleHeading2}>
                <Trans>Heading 2</Trans>
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem disabled={!state.canHeading3} checked={state.isHeading3} onCheckedChange={state.toggleHeading3}>
                <Trans>Heading 3</Trans>
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem disabled={!state.canHeading4} checked={state.isHeading4} onCheckedChange={state.toggleHeading4}>
                <Trans>Heading 4</Trans>
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem disabled={!state.canHeading5} checked={state.isHeading5} onCheckedChange={state.toggleHeading5}>
                <Trans>Heading 5</Trans>
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem disabled={!state.canHeading6} checked={state.isHeading6} onCheckedChange={state.toggleHeading6}>
              ...
              render={
                <Button size={buttonSize} tabIndex={-1} variant="ghost" className={buttonClassName}>
                  {match(state)
                    ...
                    .otherwise(() => <TextAlignLeftIcon className="size-3.5" />)}
                </Button>
              }
            />

            <DropdownMenuContent>
              <DropdownMenuCheckboxItem disabled={!state.canLeftAlign} checked={state.isLeftAlign} onCheckedChange={state.toggleLeftAlign}>
                <Trans>Left Align</Trans>
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem disabled={!state.canCenterAlign} checked={state.isCenterAlign} onCheckedChange={state.toggleCenterAlign}>
                <Trans>Center Align</Trans>
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem disabled={!state.canRightAlign} checked={state.isRightAlign} onCheckedChange={state.toggleRightAlign}>
                <Trans>Right Align</Trans>
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem disabled={!state.canJustifyAlign} checked={state.isJustifyAlign} onCheckedChange={state.toggleJustifyAlign}>
          ...
          <Button size={buttonSize} tabIndex={-1} variant="ghost" className={buttonClassName} disabled={!state.canLiftListItem} onClick={state.liftListItem}>
            <TextOutdentIcon className="size-3.5" />
          </Button>

          <Button size={buttonSize} tabIndex={-1} variant="ghost" className={buttonClassName} disabled={!state.canSinkListItem} onClick={state.sinkListItem}>
          ...
          {state.isLink ? (
            <Button size={buttonSize} tabIndex={-1} variant="ghost" className={buttonClassName} onClick={state.unsetLink}>
              <LinkBreakIcon className="size-3.5" />
            </Button>
          ) : (
            <Button size={buttonSize} tabIndex={-1} variant="ghost" className={buttonClassName} onClick={state.setLink}>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              render={
                <Button size={buttonSize} tabIndex={-1} variant="ghost" className={buttonClassName} title={t`Table`}>
          ...
          <Button size={buttonSize} tabIndex={-1} variant="ghost" className={buttonClassName} title={t`New Line`} onClick={state.setHardBreak}>
            <KeyReturnIcon className="size-3.5" />
          </Button>

          <Button size={buttonSize} tabIndex={-1} variant="ghost" className={buttonClassName} title={t`Separator`} onClick={state.setHorizontalRule}>
